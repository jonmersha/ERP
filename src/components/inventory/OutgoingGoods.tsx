  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-3xl shadow-sm border border-[var(--color-text)]/5 overflow-hidden">
      <div className="p-6 border-b border-[var(--color-text)]/5">
        <h3 className="font-serif font-bold text-lg text-[var(--color-text)]">{t('Pending Shipments')}</h3>
      </div>
      <div className="divide-y divide-[var(--color-text)]/5">
        {(pendingSOs?.length || 0) === 0 ? (
          <div className="p-12 text-center text-[var(--color-text)]/30 italic">{t('No pending sales orders to ship')}</div>
        ) : (
          pendingSOs.map(so => (
            <div key={so.id} className="p-6 hover:bg-[var(--color-text)]/[0.02] transition-colors flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-xs font-mono font-bold text-[var(--color-main)]">{t('SO #')}{so.id?.slice(0, 8)}</p>
              ...
              <div className="flex items-center space-x-8">
                <div className="text-right">
                  ...
                  <p className="font-bold text-[var(--color-text)]">{so.items?.length || 0} {t('types')}</p>
                </div>
                <button 
                  onClick={() => onShip(so)}
