    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-white rounded-2xl border border-black/5 focus:outline-none focus:ring-2 focus:ring-[#5A5A40]/20 transition-all"
          />
        </div>
        <div className="flex items-center space-x-2 bg-white px-4 py-2 rounded-2xl border border-black/5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredInventory.map(item => {
          ...
          return (
            <div key={item.id} className="bg-white p-6 rounded-3xl shadow-sm border border-black/5 hover:shadow-md transition-all group">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                  <p className="text-xs font-bold text-black/40 uppercase tracking-widest mb-1">{item.itemType === 'raw' ? 'Raw Material' : 'Finished Product'}</p>
                  <p className="text-2xl font-serif font-bold text-black">{item.quantity}</p>
                </div>
              </div>
              <div>
                <h4 className="font-bold text-lg text-black mb-1">{details?.name || 'Unknown Item'}</h4>
                <p className="text-sm text-black/40 flex items-center">
