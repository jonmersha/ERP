      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--color-surface)] rounded-3xl shadow-sm border border-[var(--color-text)]/5 overflow-hidden">
          <div className="p-6 border-b border-[var(--color-text)]/5 flex justify-between items-center">
            <h3 className="font-serif font-bold text-lg text-[var(--color-text)]">{t('Recent Receipts (GRNs)')}</h3>
          ...
          <div className="divide-y divide-[var(--color-text)]/5 max-h-[600px] overflow-y-auto">
            {(grns?.length || 0) === 0 ? (
              <div className="p-12 text-center text-[var(--color-text)]/30 italic">{t('No receipt history')}</div>
            ) : (
              grns.map(grn => (
                <div key={grn.id} className="p-4 hover:bg-[var(--color-text)]/[0.02] transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <p className="text-xs font-mono font-bold text-[var(--color-main)]">{t('GRN #')}{grn.id?.slice(0, 8)}</p>
        ...
        <div className="bg-[var(--color-surface)] rounded-3xl shadow-sm border border-[var(--color-text)]/5 overflow-hidden">
          <div className="p-6 border-b border-[var(--color-text)]/5 flex justify-between items-center">
            <h3 className="font-serif font-bold text-lg text-[var(--color-text)]">{t('Recent Shipments (DNs)')}</h3>
            <ArrowUpRight size={20} className="text-indigo-600" />
          </div>
          <div className="divide-y divide-[var(--color-text)]/5 max-h-[600px] overflow-y-auto">
            {(deliveryNotes?.length || 0) === 0 ? (
              <div className="p-12 text-center text-[var(--color-text)]/30 italic">{t('No shipment history')}</div>
            ) : (
              deliveryNotes.map(dn => (
                <div key={dn.id} className="p-4 hover:bg-[var(--color-text)]/[0.02] transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <p className="text-xs font-mono font-bold text-[var(--color-main)]">{t('DN #')}{dn.id?.slice(0, 8)}</p>
